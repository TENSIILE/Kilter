...
import { authInputChangeAction } from '../../redux/actions/AuthInput.action'
import {
  LOGIN_LOGIN,
  LOGIN_PASSWORD,
  SIGNUP_EMAIL,
...
export const AuthState = ({ children }) => {
  ...
  const onChangeInputHandler = useCallback(e => {
    dispatch(authInputChangeAction(e))
  }, [])
  ...
  }, [inputs, alert, auth, request])

  const onAsyncSignUp = useCallback(async () => {
    try {
      const { message } = await request(
        `${config.SERVER_HOST}/auth/signUp`,
        'POST',
        {
